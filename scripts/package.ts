 ...
 * the directory. Source maps are excluded: they roughly double the upload and
 * nothing in review reads them.
 *
 *   bun run package
 */
...
await $`zip -r -X ../${OUT}/${zip} . -x '*.map' -x '.DS_Store'`.cwd("dist").quiet();
