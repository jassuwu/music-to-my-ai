...
Glossary for Plainsong — a browser extension that turns streaming LLM responses into sound. ("Music to My Agents" was the working title and the repo directory still carries it; "Halo" and "Aux" were short-lived intermediate names.)

## Glossary

- **Plainsong** — the product's name: plaintext, sung. The old word for unaccompanied chant, repurposed because this extension turns plain text into song as it streams. The visual identity stays borrowed from the owner's site (jass.gg tokens; the icon square is its lime).
